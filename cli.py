...
def modify_config_interactive(config):
    ...
    # Vertex AI Settings
    print("\n" + "-" * 40)
    print("VERTEX AI SETTINGS")
    print("-" * 40)

    config["GOOGLE_CLOUD_PROJECT"] = ask_string(
        "Google Cloud Project ID",
        config.get("GOOGLE_CLOUD_PROJECT", ""),
        required=True
    )
    config["GOOGLE_CLOUD_LOCATION"] = ask_string(
        "Google Cloud Location",
        config.get("GOOGLE_CLOUD_LOCATION", "us-central1")
    )
    config["MODEL_NAME"] = ask_string(
        "Model Name",
        config.get("MODEL_NAME", "gemini-2.5-flash")
    )
    ...
    config["HANJA_IDENTIFICATION"] = ask_yes_no(
        "Identify hanja in text?",
        "y" if config.get("HANJA_IDENTIFICATION", True) else "n"
    )
    config["SAVE_NEW_ONLY"] = ask_yes_no(
        "Export only new glossary terms?",
    ...
    config["LOCAL_MODEL"] = ask_yes_no(
        "Use local ML model (instead of Gemini)?",
        "y" if config.get("LOCAL_MODEL", False) else "n"
    )

    if config["LOCAL_MODEL"]:
        config["DICT_API_KEY"] = ask_string(
            "KRDICT API Key (for dictionary checking)",
            config.get("DICT_API_KEY", "")
        )
    else:
        config["DICT_API_KEY"] = ""

    config["GUESS_HANJA"] = ask_yes_no(
        "Guess missing hanja?",
        "y" if config.get("GUESS_HANJA", False) else "n"
    )
    config["DO_CATEGORIZATION"] = ask_yes_no(
        "Categorize terms?",
        "y" if config.get("DO_CATEGORIZATION", False) else "n"
    )
    config["DO_TRANSLATION"] = ask_yes_no(
        "Translate terms to English?",
        "y" if config.get("DO_TRANSLATION", True) else "n"
    )
    config["SIMPLIFIED_CHINESE_CONVERSION"] = ask_yes_no(
        "Convert hanja to Simplified Chinese?",
    ...
    config["CHAPTERS_ANALYZED"] = ask_int("Chapters analyzed simultaneously", config.get("CHAPTERS_ANALYZED", 10), 1, 50)
    config["CATEGORIZATION_BATCH_SIZE"] = ask_int("Categorization batch size", config.get("CATEGORIZATION_BATCH_SIZE", 50), 1, 100)
    config["TRANSLATION_BATCH_SIZE"] = ask_int("Translation batch size", config.get("TRANSLATION_BATCH_SIZE", 50), 1, 100)
    config["HANJA_GUESSING_BATCH_SIZE"] = ask_int("Hanja guessing batch size", config.get("HANJA_GUESSING_BATCH_SIZE", 50), 1, 100)
    ...
    config["MAX_RETRIES"] = ask_int("Max retries on API failure", config.get("MAX_RETRIES", 3), 1, 20)
    config["RETRY_DELAY"] = ask_int("Retry delay (seconds)", config.get("RETRY_DELAY", 31), 1, 300)
...
def run_pipeline_with_config(config_path="config.json"):
    ...
    print(f"\n☁️  Vertex AI:")
    print(f"  • Project: {config.get('GOOGLE_CLOUD_PROJECT', 'NOT SET')}")
    print(f"  • Location: {config.get('GOOGLE_CLOUD_LOCATION', 'us-central1')}")
    print(f"  • Model: {config.get('MODEL_NAME', 'gemini-2.5-flash')}")
    ...
    # Validate project ID
    if not config.get("GOOGLE_CLOUD_PROJECT", "").strip():
        print("\n🔑 Google Cloud Project ID not found in config.")
        config["GOOGLE_CLOUD_PROJECT"] = input("Please enter your Google Cloud Project ID: ").strip()
    ...
    # Handle KRDICT key if local model is enabled
    if config.get("LOCAL_MODEL", False) and not config.get("DICT_API_KEY", "").strip():
...
def setup_colab():
    ...
    print("1. Create a Google Cloud project and enable the Vertex AI API")
    print("2. Add your Project ID to Colab Secrets as 'GOOGLE_CLOUD_PROJECT'")
    print("3. Upload your Korean novel text files to a folder named 'raws'")
    print("4. Run: !python cli.py --config config.json")
...
def interactive_setup():
    ...
    # Vertex AI Settings
    print("\nVertex AI Settings:")
    config["GOOGLE_CLOUD_PROJECT"] = input("Google Cloud Project ID: ").strip()
    while not config["GOOGLE_CLOUD_PROJECT"]:
        print("Project ID is required!")
        config["GOOGLE_CLOUD_PROJECT"] = input("Google Cloud Project ID: ").strip()

    config["GOOGLE_CLOUD_LOCATION"] = input("Google Cloud Location [us-central1]: ").strip() or "us-central1"
    ...
    config["SAVE_NEW_ONLY"] = input("Export only new glossary terms? [n]: ").strip().lower() == 'y'
    if config["SAVE_NEW_ONLY"]:
        print("  ⓘ Only terms not in the original glossary will be exported to Excel.")
    ...
    if config["LOCAL_MODEL"]:
        env_dict_key = os.environ.get('KRDICT_API_KEY', '')
        if env_dict_key:
            print(f"✓ Found KRDICT_API_KEY in environment ({env_dict_key[:8]}...)")
            use_env = input("Use environment variable? (y/n, default: y): ").strip().lower() or 'y'
            config["DICT_API_KEY"] = env_dict_key if use_env == 'y' else input("KRDICT API key: ").strip()
        else:
            config["DICT_API_KEY"] = input("KRDICT API key (for dictionary checking): ").strip()
    else:
        config["DICT_API_KEY"] = ""

    config["GUESS_HANJA"] = input("Guess missing hanja? [n]: ").strip().lower() == 'y'
    config["DO_CATEGORIZATION"] = input("Categorize terms? [n]: ").strip().lower() == 'y'
    config["DO_TRANSLATION"] = input("Translate terms to English? [y]: ").strip().lower() != 'n'
    config["SIMPLIFIED_CHINESE_CONVERSION"] = input("Convert hanja to Simplified Chinese? [n]: ").strip().lower() == 'y'
