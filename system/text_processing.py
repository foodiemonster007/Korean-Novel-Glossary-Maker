...
def extract_hanja_nouns_with_regex(text):
    ...
    bracketed_pattern = r'([가-힣]+)\s*\(([一-龯\u4e00-\u9fff豈舘]+)\)'
    adjacent_pattern = r'([가-힣]+)([一-龯\u4e00-\u9fff豈舘]+)'
    
    found_nouns = {}

    for hangul, hanja in re.findall(bracketed_pattern, text):
        if len(hangul) == 1:
            continue
        if len(hangul) == len(hanja):
            if hangul not in found_nouns:
                found_nouns[hangul] = {'hangul': hangul, 'hanja': hanja, 'english': ''}

    for hangul, hanja in re.findall(adjacent_pattern, text):
        if len(hangul) == 1:
            continue
        if len(hangul) == len(hanja):
            if hangul not in found_nouns:
